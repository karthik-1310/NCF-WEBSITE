...
class DataIngestion:
    ...
    def ingest_all_data(self, species_csv, frequency_csv, google_drive_csv=None):
        ...
        for _, row in species_df.iterrows():
            ...
            species = Species(
                english_name=str(english_name) if english_name else "",
                scientific_name=str(scientific_name) if scientific_name else "Unknown",
                type=str(bird_type) if bird_type else "Arboreal Birds",
                taxa=str(taxa) if taxa else "Birds",
                size=str(size) if size and not pd.isna(size) else ""
            )
        ...
        for _, row in species_df.iterrows():
            ...
            try:
                image_name = str(row.get('Image File Name', f"{english_name}.png")) if 'Image File Name' in row else f"{english_name}.png"
                # Replace NaN values with default
                if image_name == 'nan' or not image_name:
                    image_name = f"{english_name}.png"
                    
                image_link = str(row.get('Image Link', '')) if 'Image Link' in row else ''
                if image_link == 'nan' or not image_link:
                    image_link = ''
            except:
                image_name = f"{english_name}.png"
                image_link = ''
            ...
            if (not image_link or image_link == '') and image_name in drive_inventory:
            ...
            if image_link and str(image_link).lower() not in ['nan', 'none', ''] and image_name and str(image_name).lower() not in ['nan', 'none', '']:
                # Convert Google Drive link to direct URL if needed
                direct_link = convert_google_drive_link(image_link)
                
                # Ensure no 'nan' strings are passed to the database
                def clean_value(value):
                    if value is None or pd.isna(value) or str(value).lower() in ['nan', 'none', '']:
                        return ''
                    return str(value)
                
                illustration = Illustrations(
                    image_name=clean_value(image_name),
                    image_link=clean_value(direct_link),
                    species_english_name=clean_value(english_name),
                    sex=clean_value(row.get('Sex', '')),
                    breeding_status=clean_value(row.get('Breeding Status', '')),
                    subspecies=clean_value(row.get('Subspecies', '')),
        ...
        for _, row in species_df.iterrows():
            ...
            # Check for Mizo name
            if 'Mizo Name' in row and row['Mizo Name'] and not pd.isna(row['Mizo Name']):
                mizo_name = str(row['Mizo Name'])
                if mizo_name and mizo_name != 'nan':
                    name = Names(
                        species_english_name=str(english_name) if english_name else "",
                        language='Mizo',
                        name=mizo_name
                    )
                db.session.add(name)
        ...
        for _, row in frequency_df.iterrows():
            ...
            frequency = Frequency(
                english_name=str(english_name) if english_name else "",
                state=str(state) if state and state != 'nan' else "Unknown",
                district=str(district) if district and district != 'nan' else "Statewide",
                frequency_rank=frequency_rank,
                observation_count=observation_count,
                seasonality=str(seasonality) if seasonality and not pd.isna(seasonality) else ""
            )
